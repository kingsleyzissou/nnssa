...
from .constants import SR, HOP_SIZE
...
def subdivide_labels(row, padding, subdivision=0, sr=SR, hop_length=HOP_SIZE):
    ...
    segment_frames = librosa.time_to_frames(
        row.Labels, sr=sr, hop_length=hop_length) + padding
    if (subdivision > 0):
        segment_frames = [np.argmin(np.abs(row.Beat_frames - s))
...
def create_spec_windows(melspec, window, subdivision=1):
    ...
    bands = np.zeros(shape=(subdivisions, subdivision,
                            melspec.shape[0], context_window))
